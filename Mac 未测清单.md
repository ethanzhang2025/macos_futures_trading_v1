...
| **v13.27** | _本批_ | ⌘⇧S 一键保存选中画线为模板（仅 n=1 + 未锁） | ⚠️ **未测** |
| **v13.28** | _本批_ | ⌘⇧L 一键切换选中画线锁定状态（全锁→全解锁 / 否则全锁） | ⚠️ **未测** |
| **v13.29** | _本批_ | 主图截图复制到剪贴板（不弹保存对话框 · 直接 NSPasteboard） | ⚠️ **未测** |
| **v13.30** | _本批_ | ⌘⇧H HUD 显隐切换（截图前可隐藏让画面干净） | ⚠️ **未测** |
| **v13.31** | _本批_ | 多边形画线（DrawingType 第 10 类 · N≥3 点闭合 · 工具栏"完成"按钮触发） | ⚠️ **未测** |
| **v13.32** | _本批_ | anchor 拖动 OHLC 价格吸附（5px 阈值 · ⌥ 临时关闭） | ⚠️ **未测** |
| **v13.33** | _本批_ | extraPoints 顶点拖动支持（多边形 N 顶点 / Pitchfork C 点都可拖） | ⚠️ **未测** |
| **v13.34** | _本批_ | HUD 位置 4 角切换（左上/右上/左下/右下 · UserDefaults 持久化 · 右键菜单选择） | ⚠️ **未测** |
| **v13.35** | _本批_ | 文字标注下划线 isUnderline（同 isBold/isItalic 模式 · 右键 toggle · 兼容老 JSON） | ⚠️ **未测** |
