...
## v11.0 → v13.6 累积验收（2026-04-28 ~ 04-30 · 共 127 commit · L1-1/2/3/4 已通过 · 含 WP-42 画线工具完整激活 + 6 commit polish）
...
### v13.0 增量（2026-04-30 · WP-42 画线工具完整 UI 激活 · a8cc75c · 用户最后一次性 Mac 验）

- [ ] **工具栏画线工具组**：副图分段控件之后 · 8 按钮一字排列
  - [ ] cursorarrow 浏览 / 趋势线 / 水平线 / 矩形 / 平行通道 / 斐波那契 / 文字 / trash 清空
  - [ ] 激活某工具 → 该按钮蓝色背景高亮（accentColor 0.3 opacity）
  - [ ] hover 显示中文 Help tooltip（如"趋势线（双点）"）

- [ ] **趋势线**（双点 · 黄色）：
  - [ ] 选趋势线工具 → 主图 1 次点击设第 1 点（pendingDrawingPoint）→ 第 2 次点击完成画线 + 自动回浏览模式
  - [ ] 完成后线段从第 1 点画到第 2 点（黄色 1.5pt）

- [ ] **水平线**（一点 · 蓝色）：
  - [ ] 选水平线工具 → 单击主图任意位置 → 立即完成 + 自动回浏览模式
  - [ ] 主图横跨水平线 + 右侧蓝色价格标签（如 "3215.00"）

- [ ] **矩形**（双点对角 · 紫色）：
  - [ ] 双点定对角 → 紫色边框 + 半透明紫色填充（0.08 opacity）

- [ ] **平行通道**（双点主轴 · 红色）：
  - [ ] 双点定主轴 → 主轴线 + 副轴线（默认偏移 = 价格区间 5%）平行
  - [ ] 红色 1.5pt 双线

- [ ] **斐波那契回调**（双点 · 橙色）：
  - [ ] 双点定 0%/100% → 主图横跨 7 条水平线（0% / 23.6% / 38.2% / 50% / 61.8% / 78.6% / 100%）
  - [ ] 每条线左侧显示比例 + 价格（如 "50.0% 3215.00"）

- [ ] **文字标注**（一点 · 白色）：
  - [ ] 选文字工具 → 单击位置 → 立即添加默认文字 "标注"（v1 简化 · v13.1 加 InputSheet 让用户自定义）

- [ ] **清空所有**（trash 按钮）：
  - [ ] 点击 → 当前 (instrumentID, period) 的所有画线全删 · 持久化同步删

- [ ] **持久化**（按 instrumentID + period 隔离）：
  - [ ] 在 RB0 15分 上画一条趋势线 → ⌘Q 关 App → 重启 → 主图仍显示该趋势线
  - [ ] 切到 RB0 60分 → 看不到 RB0 15分 的画线（独立）
  - [ ] 切到 IF0 15分 → 看不到 RB0 15分 的画线（独立）
  - [ ] 切回 RB0 15分 → 趋势线还在

- [ ] **画线在主图坐标空间锚定**：
  - [ ] 拖拽平移主图 → 画线跟随平移（跟数据走 · 不跟屏幕走）
  - [ ] 缩放主图 → 画线随之缩放

### v13.1~v13.6 增量（2026-04-30 · WP-42 画线 polish 6 commit · 用户最后一次性 Mac 验）

#### v13.1 画线 hit-test anchor + Delete 键 + 文字 NSAlert
- [ ] 浏览模式（无激活工具）+ 画线非空 → 点击 startPoint/endPoint anchor ±15 像素阈值 → 该画线 selected（anchor 显示小圆圈 · 线宽 2.5pt 高亮）
- [ ] 选中后按 Delete 键 → 该画线删除 + 持久化同步删
- [ ] 选文字工具单击 → 弹 NSAlert（标题"文字标注" · 默认值"标注" · 确定添加 · 取消不画）

#### v13.2 SQLiteDrawingStore（M5 持久化 7→8）
- [ ] App 启动后 db 目录 `~/Library/Application Support/FuturesTerminal/db/` 出现第 8 个文件 `drawings.sqlite`
- [ ] 画线后 → ⌘Q 关 App → 删除 UserDefaults 之前的 drawings.* keys（`defaults delete app_id`）→ 重启 → 画线仍保留（说明读 SQLite 不读 UserDefaults）

#### v13.3 双点画线第二点 hover 实时预览
- [ ] 选趋势线 → 第 1 次点击设第 1 点 → 鼠标移动 → 实时显示第 1 点到当前 hover 位置的虚线预览
- [ ] 选矩形 / 平行通道 / 斐波那契 → 同样 hover 预览（虚线）
- [ ] 第 2 次点击完成 → 虚线变实线 + 自动回浏览模式
- [ ] 鼠标移出主图 → 虚线消失（onContinuousHover ended）

#### v13.4 hit-test 线段任意点击（之前仅 anchor）
- [ ] 浏览模式 → 点击趋势线**线段中间任意位置**（不只 anchor）→ 该画线 selected
- [ ] 点击水平线任意 x 位置 → selected
- [ ] 点击矩形 4 边任意位置 → selected
- [ ] 点击平行通道主轴或副轴任意位置 → selected
- [ ] 点击斐波那契 7 档任意一条线 → selected
- [ ] 阈值 8 像素 · 距离过远 → 不选中

#### v13.5 右键上下文菜单
- [ ] 选中画线后 → 主图右键 → 显示菜单：「删除选中画线」（红色）+「编辑文字…」（仅 .text 类型）+「取消选中」
- [ ] 「删除选中画线」→ 该画线删 + 持久化同步
- [ ] 「编辑文字…」（.text 画线）→ 弹 NSAlert 显示当前文字 → 改后确定 → 文字更新
- [ ] 未选中时右键 → 显示「（无选中画线 · 点击画线选中后再右键）」提示

#### v13.6 复制画线 + Inspector 浮窗
- [ ] 选中画线后右键 →「复制画线（⌘D）」→ 主图出现新画线（偏移 +20 bar / 价格区间 5%）+ 自动选中新画线
- [ ] 选中画线后按 ⌘D 键 → 同上效果
- [ ] 主图右下角 **Inspector 浮窗**（仅 selected 时显示）：
  - [ ] 类型中文标签（如"趋势线"/"水平线"/"矩形"/"平行通道"/"斐波那契"/"文字标注"）
  - [ ] 起点 bar+价 / 终点 bar+价（如有）/ 文字内容（仅 .text）/ 通道偏移（仅 .parallelChannel）
  - [ ] 操作提示"⌘D 复制 · Delete 删除 · 右键编辑"
  - [ ] 右上角 ✕ 关闭按钮 → 取消选中

### Mac 验收 L1 进度表（v13.6 末）
