...
fileprivate func drawingTypeLabel(_ type: DrawingType) -> String {
    ...
    case .fibonacciChannel: return "斐波通道"
...
struct ChartContentView: View {
    ...
    private func distanceToDrawing(_ p: CGPoint, drawing: Drawing, in size: CGSize) -> CGFloat {
        ...
        case .fibonacciChannel:
            // v17.18 A4.5 · 斐波通道 hit test · 7 条 fib 比例平行线段最小距离
            guard let end = drawing.endPoint, let offset = drawing.channelOffset else { return .infinity }
            let s = drawing.startPoint
            var minD: CGFloat = .infinity
            for ratio in FibonacciLevels.standard {
                let priceShift = offset * ratio
                let lineS = screenPoint(DrawingPoint(barIndex: s.barIndex, price: s.price + priceShift))
                let lineE = screenPoint(DrawingPoint(barIndex: end.barIndex, price: end.price + priceShift))
                minD = min(minD, Self.pointToSegmentDistance(p, lineS, lineE))
            }
            return minD
        ...
        case .fibonacciFan:
            // v15.87 斐波那契扇形 · 3 条核心射线（38.2/50/61.8）的最小线段距离 · 与渲染共用同一 t 延伸
            guard let end = drawing.endPoint else { return .infinity }
            let a = screenPoint(drawing.startPoint)
            let endX = screenPoint(end).x
            let prices = DrawingGeometry.fibonacciFanTargetPrices(for: drawing)
            var minD: CGFloat = .infinity
            for price in prices {
                let target = screenPoint(DrawingPoint(barIndex: end.barIndex, price: price))
                let targetAtEndX = CGPoint(x: endX, y: target.y)
                let dx = targetAtEndX.x - a.x
                let dy = targetAtEndX.y - a.y
    ...
    private func handleDrawingTap(at location: CGPoint, in size: CGSize) {
        ...
        if tool.needsTwoPoints {
            if let firstPoint = pendingDrawingPoint {
                ...
                let channelOffset: Decimal? = (tool == .parallelChannel || tool == .fibonacciChannel)
