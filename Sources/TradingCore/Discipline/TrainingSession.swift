...
public enum TrainingScorer {
    ...
    /// v16.147 · 5 维改进 plan · 弱项展开后的 5 步具体行动建议（trader 看完即可执行）
    /// score 参数预留：将来可按分数差异给不同力度建议（当前固定 5 步 · 已足够覆盖）
    public static func improvementPlan(for dim: TrainingSubScores.Dimension, score: Int) -> [String] {
        switch dim {
        case .pnl:
            return [
                "复盘最近 3 笔亏损单 · 开仓信号是否清晰？",
                "等待 RSI / MACD / 形态多重确认再入场",
                "锁定止盈：达到 1:2 风险收益比立即兑现",
                "减少日内试探单 · 单周交易频次降一半",
                "写下 3 条本周专属规则（如：跌破 MA20 不开多）"
            ]
        case .discipline:
            return [
                "开盘前写交易计划 · 入场 / 止损 / 止盈三价位",
                "设置硬止损 OCO 单 · 不手动撤改",
                "持仓时长上限 2h · 超时强平",
                "每笔单后立即记录违规类型（冲动 / 移止损）",
                "每周回顾 RulesPanel · 累积 ≥ 3 条违规强化规则"
            ]
        case .winRate:
            return [
                "减少 50% 入场次数 · 只做 A+ 级信号",
                "信号源对照表：MA20 + RSI + 量能 三选 ≥ 2",
                "日内做趋势 · 不猜顶底反转",
                "入场即设止损 · 错误信号 5 分钟内止损出局",
                "周末回顾盈利单 · 找共性 → 沉淀 3 条入场标准"
            ]
        case .risk:
            return [
                "单笔风险硬上限 = 总资金 × 1%（固定）",
                ...
                "止损不动 · 浮亏不加仓（反向金字塔禁止）",
                "单日最大亏损 ≤ 3% · 触及强制收工",
                "每月回顾 worst loss · 找共性形态规避"
            ]
        case .efficiency:
            return [
                "减少日内交易次数 · 一笔吃透行情",
                "持仓时长 ≥ 30 分钟 · 短于 5 分钟的单复盘",
                "信号过滤：只做 ATR > 平均的高波动时段",
                "入场前问：本笔预期目标 ≥ 多少跳？",
                "月度回顾平均每笔贡献 · 低于阈值的形态弃用"
            ]
        }
    }
