...
### ✅ WP-42 · 画线工具 v1 · 数据模型层完成 2026-04-24 b41acc9 + UI 完整激活 v13.0 完成 2026-04-30 a8cc75c
...
| 2026-04-28 | v11.0 | M5 持久化第 1-4 批（journal / alertHistory / watchlistBook / workspaceBook / kline 5/7 store 真接入 MainApp）· WP-41 v3 收官 41 指标增量 · simplifier 流程位置修正 · 14 commit | v11.0 末距 Legacy ~92-93% · 5 大 P1+P2 工作流 + WP-55 P3 完整 + M5 5/7 持久化 |
| 2026-04-29 | v12.0 | M5 持久化 7/7 全闭环（alertConfig + AnalyticsService 6/10 事件）· WP-63 文华 .wh 公式批量导入 ✅ DoD（WhImporter + 20 公式 + 第 20 个真数据 demo MaiYuYanWhDemo）· WP-64 文华自选文本导入 ✅ DoD（WatchlistImporter + 50+ 合约 5 组无丢失）· AlertEvaluator 真 e2e wiring（K 线 close 假 Tick · 真 Tick Stage B CTP 替换）· 视觉迭代 13 项（红绿蜡烛 + 5×5 网格 + 十字光标 + OHLC 浮窗 + 黄色轴边浮标 + 顶部大字当前价 + MA 5 色染色 + segmented 工具条 + 全局深色 + defaultSize）· 评估发现 P0 全修（AlertHistory 写库 / 空 trades 持久化 / K 线 race / history 加载语义对齐）· Mac 编译批量修复（Linux 跳 SwiftUI 未暴露 9 类问题：typealias Alert / Decimal / try? warning / 参数顺序 / compactMap 推断 / .gitignore）· 2 视觉 bug 修（回放未启动大绿块 + 默认拖拽无效 pan 范围 0）· ReviewWindow 8 图自适应布局修（adaptive + minHeight）· 23 commit | v12.0 末距 Legacy ~95-96% · 视觉接近专业终端 |
| 2026-04-29 | v12.1 | L1-5 Mac 验收 bug 修（Watchlist Mock 9 合约含未在 supportedContracts 内的 HC0/I0/AG0 → 改 4 组按板块分类 RB0/IF0/AU0/CU0 全部支持）· 真 preSettle 接入（ChartScene 顶部当前价大字 baseline 升级 Sina 实时昨结算 · fallback visible 周期首根 · 与文华行情显示口径一致）· 2 commit | L1-5 切机验前修 + 真前结算视觉精度提升 |
| 2026-04-29 | v12.2 | 第 21 个真数据 demo SinaMonthlyContractDemo 探索（10 合约 × 3 端点 · 30 真网络请求 · 关键发现：K 线端点 100% 支持月份合约 · 实时报价仅大写 + 非 I 字母支持）· 扩 MarketDataPipeline.supportedContracts 4→8（4 主连续 + 4 active 月份合约 rb2609/i2609/au2606/IF2605 · 2026-04-29 主力月）· WatchlistWindow Mock 改 2 组 8 合约（主力月份 + 主连续）· footerHint 文案更新 · ChartScene 工具条 Picker / 双击切图 guard / fetchPreSettle / Alert 文案 自动跟随 · 解锁场景：Watchlist 真持仓盯盘 + WatchlistImporter 文华自选月份合约可用 · 3 commit（含 demo / 文档对齐 v12.2 章节） | active 月份合约盯盘解锁 · v12.0~v12.2 共 25 commit · 100 commit 整百里程碑 |
| 2026-04-29 | v12.3 | 第 22 个真数据 demo SinaQuoteWorkaroundDemo 修正 v12.2 错误结论（之前"i 字母不支持"实为"小写需 uppercase"）· W1 大小写转换 100% 解决 active 月份合约实时报价（rb2609→RB2609 / i2609→I2609 / au2606→AU2606 全 OK · 真实 last + preSettle + oi + vol · I2509 失败因已交割与字母无关）· SinaMarketData.fetchQuotes 单行修复（URL 自动 uppercase · SinaQuote.symbol 保留原大小写 · 调用方无感）· 自动跟随：ChartScene fetchPreSettle 对 rb2609/i2609/au2606 拉到真 preSettle（v12.2 fallback 实质降级为兜底）· Watchlist 行情列表对小写合约拉到真行情（清掉"—"占位）· 2 commit（含 demo / 修复 + 文档 v12.3 章节） | v12.2 月份合约实时报价完整解锁（除已交割合约走 K 线兜底待 v12.4） |
| 2026-04-29 | v12.4 | Watchlist 行情列表接 SinaMarketData 真行情（自此 ⌘L 自选合约面板不再显示 MockQuote 静态数据）· 加 @State quotes / quoteFetchTask · .task 末尾 startQuoteFetch（5s 周期 fetch 全部 book 内合约 · 失败 silent 保留旧值 · onDisappear cancel）· instrumentRow 改 3 处 MockQuote → priceText / changePctText / openInterestText 3 helper · 真值 fallback Mock（首次失败 / 离线兜底）· 涨跌幅 "+%.2f%%" 前缀 "+/-" 兼容现有涨跌色（hasPrefix "-" → green）· 持仓量 ≥1M 用 M / ≥1K 用 K 自动格式 · WatchlistImporter（WP-64）真闭环：用户从文华导入的月份合约 → Watchlist 直接看真涨跌 + 真持仓 · 1 commit（含代码 + 文档 v12.4 章节） | 自选合约面板真行情盯盘 · WP-64 真闭环 |
| 2026-04-29 | v12.5 | W4 兜底实施 SinaMarketData.fetchQuotesWithFallback（基于 SinaQuoteWorkaroundDemo 3b2cb7e 验证 W4 K 线 5min 末根伪实时）· 实时端点失败的合约自动走 K 线兜底（fetchQuoteFromKLine helper 构造 partial SinaQuote · 含真 last/open/high/low/close/oi/volume · 缺失 bid/ask/preSettle/settle 置 0）· WatchlistWindow startQuoteFetch 改用 fetchQuotesWithFallback · 已交割合约（如 I2509）+ Sina 抖动场景：last + oi 真值不再纯 Mock · 涨跌幅缺 preSettle 信号时仍 Mock fallback（changePctText guard q.preSettlement > 0）· 渐进 degradation 优于 v12.4 全 Mock fallback · 1 commit（含代码 + 文档 v12.5 章节） | Watchlist 容错性提升 · 已交割合约 + sina 抖动可显示 last + 持仓量真值 |
| 2026-04-30 | v12.6 | 第 23 个真数据 demo SinaKLineGranularityDemo 探索（2 合约 × 5 type · 10 真网络请求 · 实测发现 Sina K 线端点 type=1/5/15/30/60 全部支持 · 修正 ChartScene 原注释"Sina 不支持的 fallback 15min"错误结论）· SinaMarketData 加 fetchMinute1KLines + fetchMinute30KLines · SinaMarketData+Provider historicalMinute 加 case 1 / 30 · ChartScene fetchHistoricalKLines switch 完整化（minute1 / minute15 / minute30 走真 type · 不再静默 fallback 15min）· 修复隐性 bug：用户切到 1min/15min/30min 周期时 UI 显示选中周期但数据是 15min（数据/UI 不一致）· 1 commit（含 demo / Package target / SinaMarketData 扩 / Provider 扩 / ChartScene 修 + 文档 v12.6 章节） | 多周期数据/UI 一致性修复 · 全粒度 K 线真支持 |
| 2026-04-30 | v12.7~v12.13 | Mac 切机批验回合 1 hotfix 阶段（用户 swift run MainApp 实测发现）：① v12.7 UDS 实盘路径 intervalMinutes 加 case 1/30（v12.6 仅修回放路径 · 漏 UDS）② v12.8 UDS 加 daily/weekly/monthly historicalDaily 路径（之前漏 · 切日 K → snapshot 空 → loadMockFallback 加载 5000 根 1min Mock 假数据）③ v12.9 cacheMaxBars 改 static func(for:period) 按周期动态（1min 10000 / 5min 5000 / 15min 3000 / 30min 2000 / hour1 1500 / daily 1500 · 让小周期密度大 + 大周期合理跨度）④ v12.10 KLineCrosshairView 时间格式按 period 动态（用户反馈日 K 显示 04-30 00:00 缺年份）⑤ v12.11 折衷方案（1/5/15分 MM-dd HH:mm / 30/60分 yy-MM-dd HH:mm / 日周 yyyy-MM-dd / 月 yyyy-MM）⑥ v12.12 回放控制条播放暂停按钮 state-based 高亮（playing 蓝底白字）⑦ v12.13 ReplayBarButtonStyle 统一蓝底白字反馈（pressed/active 蓝高亮 · 4 按钮统一） · 7 commit | Mac 实盘验收 hotfix 闭环 · 实盘多周期 K 线全粒度 + UI 时间格式一致 + 控制条视觉清晰 |
| 2026-04-30 | v12.14~v12.19 | Mac 切机批验回合 2 一气呵成补完 6 项功能（用户要求完整版批量 Mac 验）：① v12.14 副图扩 4 选项（之前仅 MACD/KDJ · 加 RSI 14 0~100 视野 + 70/50/30 参考线 · 加成交量 0~max 视野 + 涨红跌绿 + K/M 自动格式 + HUD 染色）② v12.15 SinaQuote 加 priceBaseline 计算属性（商品昨结算 / 金融昨收近似 语义清晰 · 行为等价）③ v12.16 主力月动态计算 DominantMonthCalculator（27 品种规则 · 半年自动续期 · 实测 2026-04-29 与 oi 排序吻合 rb2609/i2609/au2606/IF2605 · supportedContracts 改 static var computed · +7 测试 +1 suite）④ v12.17 WatchlistImporter UI 入口（⌘L sidebar 加导入按钮 · NSOpenPanel + alert 预览 + merge 确认 · WP-64 完整真闭环）⑤ v12.18 WhImporter UI 入口（主菜单工具→导入文华公式 ⌘⇧I · NSOpenPanel + WhImporter.importAll + NSAlert 编译报告 · WP-63 完整真闭环）⑥ v12.19 WP-44 周期键盘快捷键 ⌘1-6（隐藏 Button 组 + .keyboardShortcut · ⌘1=1分 / ⌘2=5分 / ⌘3=15分 / ⌘4=30分 / ⌘5=60分 / ⌘6=日）· 6 commit | 副图全选项 + 主力月自动 + 文华真闭环 + 周期键盘快捷键 · 用户 Mac 一次性批验完整版本 |
| 2026-04-30 | v13.0 | **WP-42 画线工具 v1 完整 UI 激活**（之前数据模型 v9.0 完成 b41acc9 · UI 0% 是 Stage A 距 Legacy 主要差距 · 5-7 天大动作一气呵成）· Step 1 ChartCore/Bridging/DrawingsOverlayView SwiftUI Canvas 6 种画线（趋势线/水平线含价格标签/矩形含半透明填充/平行通道两线/斐波那契 7 档 0-100%+比例标签/文字标注 · 黄蓝紫红橙白 6 配色 · 选中 anchor 点高亮 · pending 虚线预览）· Step 2 ChartScene 工具栏画线工具组 8 按钮（浏览 cursorarrow + 6 工具 SF Symbol + trash 清空 · 激活蓝色背景 + 中文 Help tooltip）· Step 3 ChartContentView 接 4 个 @Binding · chartMainArea ZStack 加 DrawingsOverlayView（在十字光标上/HUD 下）+ drawingClickCaptureLayer 仅工具激活时透明 contentShape onTapGesture · Step 4 handleDrawingTap 双点画线 pending 第一点 / 单点画线立即完成 / 完成回浏览 · screenToDataPoint 屏幕→数据空间（barIndex+price）转换 · Step 5 持久化 UserDefaults JSON v1（按 instrumentID + period 隔离 · drawings.$id.$period key · isDrawingsLoaded 守卫避免初始 [] 误覆盖 · .task 重载 + .onChange save）· 1 commit | Stage A 主要差距闭环 · 距 Legacy ~98%（之前 ~96-97%）· 画线 v1 完整流程：选工具 → 点击 → 显示 → 切合约/周期独立 + 重启保留 |
| 2026-04-30 | v13.1~v13.6 | WP-42 画线 polish 6 commit 一气呵成（用户要求不切机不测试继续干活）：① v13.1 hit-test anchor 选中（点 anchor ±15 像素 selectedDrawingID）+ Delete 键删除 + 文字 NSAlert 自定义输入（之前 v13.0 hardcode "标注"）② v13.2 持久化升级 UserDefaults JSON → SQLiteDrawingStore（M5 持久化 7→8 store · 协议 + InMemory + SQLite 双实现 · 复合主键 instrument_id+period · UPSERT · +12 单测 +2 suite）③ v13.3 双点画线第二点 hover 实时预览（onContinuousHover 跟踪 + pendingPreviewDrawing 构造虚线 · 4 双点类型实时跟随）④ v13.4 hit-test 升级线段任意点击（之前仅 anchor · 阈值 8 像素 · 6 类公式：线段距离 / 水平线 y 距离 / 矩形 4 边 / 通道双轴 / fib 各档 / text 位置 · 加 pointToSegmentDistance 经典投影夹钳）⑤ v13.5 右键上下文菜单（删除 destructive / 编辑文字 NSAlert / 取消选中 · selectedID 守卫）⑥ v13.6 复制画线（⌘D + 右键 · 偏移 +20 bar/价格区间 5% 自动选中新 + Inspector 浮窗（bottomTrailing overlay 显示类型中文+起终点+文字+通道偏移+操作提示） · 6 commit | WP-42 完整可用 v1 体验：6 种画线 + 工具栏 + 单/双点击 + hover 预览 + hit-test + 选中态 + Delete/⌘D + 右键菜单 + 复制 + Inspector + SQLite 持久化 |
| 2026-04-30 | v13.8~v13.10 | WP-42 画线 polish v2 三批一气呵成（用户要求所有功能一次写完 Mac 一次验）：① **v13.8 颜色/线宽自定义**（Drawing 加 strokeColorHex: String?+strokeWidth: Double? 两个可选字段 · 老 JSON 缺字段自动 nil · 工具栏画线工具组之间加 ColorPicker 28pt + Stepper 0.5~5.0 步进 0.5 默认 1.5 · 新建画线时 handleDrawingTap/promptTextInput 应用工具栏当前色/宽 · 右键菜单加 3 项"应用当前颜色（n 个）/ 应用当前线宽 X.X pt（n 个）/ 恢复默认（n 个）"批量改 · DrawingsOverlayView effectiveColor/effectiveWidth helper 优先用 drawing 字段缺省回退类型默认 · colorFromHex 解析 6 位 RGB · NSColor.usingColorSpace(.sRGB) 桥接 SwiftUI Color → hex · +5 单测覆盖默认值/自定义往返/老 JSON 兼容/仅色/仅宽）② **v13.9 多选画线**（selectedDrawingID: UUID? → selectedDrawingIDs: Set<UUID> · ⇧ 加选 toggle / 普通点击单选替换 · NSEvent.modifierFlags.contains(.shift) 检测 · Delete/⌘D 改批量操作 · 右键菜单全部加（n 个）后缀 · 编辑文字仅 n=1 + .text 时显示 · drawingInspector 多选时显示数量 + 操作提示 / 单选时显示色+宽 · DrawingsOverlayView selectedID → selectedIDs 多选 anchor 高亮 · duplicateDrawing 拆 duplicatedDrawing return helper 供批量复制用）③ **v13.10 拖动 anchor**（drawingHitTestLayer 改 DragGesture(minimumDistance: 0) 一手势包揽 tap+drag · onChanged 第一帧探查起点击中已选画线 anchor ±15 像素 · 距离 ≥4 像素 + 命中 → 进入拖动模式实时改 startPoint/endPoint · 释放距离 < 4 → 当 tap 走 handleSelectionClick · findAnchorAt + anchorScreenPoint helper · **关键手势冲突修复**：panGesture onChanged 加 anchorDragTarget != nil 立即 return · onEnded 用 dragStartViewport != nil 判定 pan 实际跑过避免误触发惯性 · drawingHitTestLayer 命中 anchor 时立即 cancel inertiaTask 避免主图惯性继续滑 anchor 漂移）· code-simplifier 1 轮过审（修了 simultaneousGesture(panGesture) 与 child DragGesture 同时跑导致 anchor 漂移真 bug）· 3 commit | WP-42 v1 体验完结：颜色/线宽/多选/拖动 anchor 全闭环 · v13.7+ backlog 全清 |
| 2026-04-30 | v13.11~v13.14 | WP-42 画线 polish v3 四批继续推（用户要求"做完某个任务才停下" + 一气呵成不切机）：① **v13.11 锁定画线**（Drawing 加 isLocked: Bool? + locked computed property · 渲染层 drawAnchorMarker 锁定时 anchor 用 SF Symbol lock.fill 替代圆点视觉一眼区分 · findAnchorAt + Delete shortcut + 右键菜单"删除/编辑/改色/改宽/恢复默认"全加 .disabled(allLocked) 守卫 · ⌘D 复制副本不继承锁 isLocked: nil 让用户立即可调 · drawingContextMenu 锁定/解锁 3 分支 全锁/混合/全未锁智能切换 · Inspector 显示锁标志 + 操作提示改"右键解锁后可拖动/删除"）② **v13.12 文字字体大小**（Drawing 加 fontSize: Double? · 渲染 d.fontSize ?? 12 · 工具栏 .text 工具激活时条件显示 Stepper 8~32 步进 1 默认 12 · 创建时应用 currentFontSize · 右键菜单 .text+n=1 加"修改字号…" NSAlert 输入数字 clamp 8~32 · ChartContentView 加 @Binding currentFontSize 闭环 · code-simplifier 1 轮发现并修了 ChartContentView Binding 缺失真 bug：Linux 跳过 #if os(macOS) 没暴露 · Mac 切机会立即编译错）③ **v13.13 椭圆画线**（DrawingType 加 .ellipse · 7 → 8 type · 工厂方法 Drawing.ellipse · 工具栏 SF Symbol circle 按钮 · drawEllipse 半透明填充+描边 青色 · hit-test 椭圆周距离公式 归一化半径×min(a,b) · pendingPreviewDrawing/distanceToDrawing/drawingTypeLabel 全 case 加齐）④ **v13.14 测量工具**（DrawingType 加 .ruler · 第 8 类 · 工厂方法 Drawing.ruler · 工具栏 SF Symbol ruler · drawRuler 强制虚线 + 中点标签 价格差/百分比/bar 数 · 金色 · hit-test 同 trendLine 点到线段距离）· 测试 +6 单测 +1 suite（factoryTypes 6→8 · twoPointsContract 加 ellipse/ruler · roundTrip 8 类型 · DrawingExtraFieldsTests 5 测试 · ellipseFactory + rulerFactory + isLocked 3 + fontSize + legacy 兼容）· 4 commit | DrawingType 第 7/8 类 · 锁定/字号 字段 · WP-42 共 8 画线类型 + 7 polish + 4 polish v3 · 距 Legacy ~99% |
| 2026-04-30 | v13.15~v13.17 | WP-42 画线 polish v4 三批（用户"不再让我确认 · 把能做的都做完 · 除非任务做完才停"）：① **v13.15 透明度自定义**（Drawing 加 strokeOpacity: Double? 字段 · 0.0~1.0 clamp · ColorPicker supportsOpacity true 显示 alpha 滑块 · alphaComponent helper 用 NSColor.alphaComponent 提取 · handleDrawingTap/promptTextInput 4 路径全应用 · applyStrokeColor 加 opacity 参数同时改 · resetStrokeStyle/duplicatedDrawing 全闭环 · Inspector 显示"透：XX%"仅 < 1.0 时 · 渲染 baseOpacity * userOpacity 叠加 isPending 0.6）② **v13.16 画线模板**（保存常用 / 跨合约复用 · 新加 Sources/Shared/Drawings/DrawingTemplate.swift 数据模型 · ChartScene UserDefaults JSON 持久化 key drawingTemplates.v1 · 全局共享不按合约/周期 · 工具栏 SF Symbol star Menu 按钮 · 列表+保存+删全部 · NSAlert 输入名称 · instantiateTemplate 锚点重定位 baseBar = bars.count-30 价格保留 · 选用 UserDefaults 而非 SQLite store 简化 - 模板量小 + 全局共享）③ **v13.17 Andrew's Pitchfork**（DrawingType 第 9 类 · 3 点输入 A handle / B upper / C lower · Drawing 加 extraPoints: [DrawingPoint]? 字段 + pointsNeeded computed 1/2/3 · 工厂 pitchfork(handle:upper:lower:)· 工具栏 SF Symbol tuningfork · 草绿色 · drawPitchfork 中线 + 上下平行 + BC 虚线提示 · pitchforkExtensionScale 静态 helper 同时 dx/dy 双向取 min t · @State pendingExtraPoints + handlePitchforkTap 三段式输入 · pendingPreviewDrawing phase 2 完整 3 线预览 · distanceToDrawing 与渲染 t 严格对齐 · drawAnchorPoints 加 extraPoints anchor 渲染 · duplicatedDrawing 复制 extraPoints 同 offset 平移保形 · code-simplifier 修了 3 真 bug：trash 漏清 pendingExtraPoints / 渲染 t 只看 dx 单向漏 dy / hit-test 与渲染范围不对齐导致延伸部分点不到）· 测试 +5 +1 suite · 共 858/205（v13.14 末 853/205 → +5 +1 suite · 0 回归）· 3 commit | WP-42 共 9 画线类型 + 14 polish · 字段 +3（isLocked/fontSize/strokeOpacity/extraPoints）· 距 Legacy ~99%（核心差距收窄到 CTP/CloudKit）|
| 2026-04-30 | v13.18~v13.20 | 跨模块集成 + 副图增强 3 批（用户"做完任务才停 · 把能做的都做完"）：① **v13.18 画线→预警联动**（WP-42+WP-52 集成 · ChartScene drawingContextMenu 加"为此画线创建预警…"项 · 仅 .horizontalLine + n=1 + 未锁 · NSAlert 输入预警名称 · post NotificationCenter .alertAddedFromChart · AlertWindow .onReceive 防重复 ID 后 alerts.append → 自动 onChange save SQLiteAlertConfigStore + sync evaluator · 横跨 3 文件 ChartScene/AlertWindow/Notification.Name extension）② **v13.19 副图多选叠加**（selectedSubIndicator: SubIndicatorKind → selectedSubIndicators: Set · 工具栏 Picker(.segmented) → Menu Toggle 多选 · 至少 1 个守卫不允许全空 · ChartContentView subIndicatorKind → subIndicatorKinds: [SubIndicatorKind] · 副图区 ForEach VStack vertical stack · perSubHeight = subChartTotalHeight/count · 1pt 分割线 · 埋点改 sorted joined kinds string）③ **v13.20 主图/副图分割条拖动**（@State subChartTotalHeight + dragStartSubHeight · SubChart enum defaultHeight 160 / minHeight 80 / maxHeight 480 · mainSubDivider 4pt + onHover NSCursor.resizeUpDown.set（**code-simplifier 修复 NSCursor.push/pop 栈泄漏真 bug**）+ DragGesture(minimumDistance: 0) · 向上拖副图变高 · 不持久化 v1）· 共 858/205（v13.17 末 858/205 同 · 这 3 批纯 UI 改动无新单测）· 3 commit | WP-42 + WP-52 跨模块联动 · 副图从单选 1 个升级到多选 4 个 + 用户可调高度 · 距 Legacy ~99%（持续推进）|
| 2026-04-30 | v13.21~v13.30 | 状态持久化 + 键盘快捷键 + 视觉/截图工具 10 批一气呵成（用户"不再让我确认 · 把能做的都做完 · 除非任务做完才停"）：① **v13.21** 副图选择 + 高度 UserDefaults 持久化（subIndicators.v1 + subChartHeight.v1 · isSubPrefsLoaded 守卫）② **v13.22** viewport 缩放级别按合约+周期记忆（key viewport.v1.<id>.<period> · RenderViewport JSON · 1s 节流 · clamp 防越界）③ **v13.23** viewport 键盘快捷键（⌘= 放大 / ⌘- 缩小 / ⌘0 重置 120 根 / ←/→ pan 5 根 / ⇧←/⇧→ pan 25 根加速 · 全部 cancel inertiaTask）④ **v13.24** K 线主图右键扩展（无选中时显示重置/放大/缩小 + 复制可见区 OHLC CSV → NSPasteboard 格式 time,o,h,l,c,volume）⑤ **v13.25** 主图截图导出 PNG（ImageRenderer 1280x720 Retina 2x · NSImage→PNG · NSSavePanel）⑥ **v13.26** 文字标注 isBold + isItalic 字段（兼容老 JSON · 右键 toggle 互斥显示 · DrawingsOverlayView Font.Weight + .italic() modifier · +1 测试）⑦ **v13.27** ⌘⇧S 一键保存选中画线为模板（仅 n=1 + 未锁）⑧ **v13.28** ⌘⇧L 一键切锁（全锁→全解锁 / 否则→全锁）⑨ **v13.29** 主图截图复制到剪贴板（NSPasteboard.writeObjects 不弹文件对话框）⑩ **v13.30** ⌘⇧H HUD 显隐切换（截图前可隐藏让画面干净）· 共 859/205（v13.20 末 858/205 → +1 v13.26 测试）· 4 commit | 状态持久化 + 5 类键盘快捷键齐全（⌘1-6 周期/⌘=/-/0 缩放/←→pan/Delete⌘D 选中/⌘⇧SLH 画线快捷）· 距 Legacy ~99%+（核心差距收窄到 CTP/CloudKit · 大量 UX polish）|
