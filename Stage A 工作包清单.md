...
5. **文档字段约定**：每个 WP 标注「时点 · 负责 · 依赖 · 交付 · DoD · 锚点」六要素
...
- **交付**：合规边界纪要，必须专项问清以下 4 点：
  ...
  2. 麦语言兼容话术红线 + 用户协议框架
  3. **CloudKit 境外存储与《个保法》第 40 条个人信息出境规则的冲突**（详 D4 G1）
  4. **期货软件是否需向中期协备案 + 模拟训练合规边界**（详 D4 G6）
- **DoD**：1 份律师咨询纪要归档，4 项均有明确结论
- **锚点**：D2 §9 Phase 0、D1 §5、D4 G1/G6
- **预算**：¥1-3 万
...
- **交付**：
  - 《用户协议》《隐私政策》《个保法合规说明》
  - 《境内数据存储 + 加密实现说明》（分级加密详 D4 G11：Keychain / SQLCipher / 明文）
  - Apple 金融类 App 审核材料（律师函 / Demo 账号 / Review Notes 中英双语）
  - **Apple 金融类审核拒审 Top 10 对策清单**（详 D4 G7）
- **DoD**：律师审过 + Apple 审核材料就绪 + 拒审 Top 10 对策落地
- **锚点**：D2 §9 Phase 1、D4 G7/G11
...
### ⬜ WP-21 · CTP SimNow 行情订阅 PoC
- **时点**：M1 Week 2-3
- **负责**：你
- **依赖**：SimNow 账号（免费申请）
- **交付**：CTP Level 1 行情订阅 demo、可查看全市场合约行情推送
- **DoD**：订阅成功 + 数据落到 SwiftUI demo 显示
- **锚点**：D2 §7 Week 2-3、§4 技术栈
...
- **时点**：M2-M4
- **负责**：你
- **依赖**：WP-20 PoC、WP-31 Legacy KLineChartView 融合
- **交付**：全市场合约实时行情、多周期切换（1m/5m/15m/30m/1h/4h/日/周/月）、多窗口布局（最多 6 同屏）
- **DoD**：10 万 K 线 60fps、延迟 <16ms、首次交互 <100ms、内存 <500MB
- **锚点**：产品设计书 §3.1 模块①、D2 §2

### ⬜ WP-41 · 指标库 v1 · 56 个
- **时点**：M3
- **负责**：你（AI 批量生成骨架 + 对照 TradingView/文华手动校验）
- **依赖**：FormulaEngine（Legacy）
...
- **负责**：你
- **交付**：M1-M6 所有功能数据结构预留 CloudKit 字段；M7 上线自选 / 模板 / 日志三项同步
- **DoD**：两台设备双向同步无冲突
- **锚点**：D2 §2、D2 §6 M7 milestone
...
- **锚点**：D2 §2 M7-M9、Legacy迁移融合方案.md

### ⬜ WP-63 · 文华麦语言公式（.wh）导入（源自 D4 G4）
- **时点**：M8（与 WP-62 同期）
- **负责**：你
- **依赖**：WP-62 麦语言引擎
- **交付**：`.wh` 文本公式文件批量导入 + 解析 + 编译；失败公式给出明确错误定位
- **DoD**：文华典型公式 20 个测试用例全绿
- **锚点**：D4 G4、Legacy迁移融合方案.md

### ⬜ WP-64 · 文华自选列表导入（源自 D4 G4）
- **时点**：M8
- **负责**：你
...
### ⬜ WP-84 · CloudKit 合规方案落地（源自 D4 G1）
- **时点**：M4-M5（律咨结论后启动）
- **负责**：你
- **依赖**：WP-02 律师咨询结论
- **交付**：按 D4 G1 三备选任一落地：
...
- **交付**：
  ...
  - **多设备绑定**（D4 G12）：Pro 3 台激活上限、device_id 后端维护、超限踢出最久未用、90 天自助重置
- **DoD**：沙盒环境订阅链路完整 + 后端订阅状态写入正确 + 设备上限生效
- **锚点**：D2 §3 支付通道、D4 G12、产品设计书 5.6
...
### ⬜ WP-94 · 手动开票 SOP 与财务表模板（源自 D4 G5）
- **时点**：M6 上线时启用
- **负责**：合伙人（主）+ 你
...
### ⬜ WP-95 · M6 Pre-launch Checklist 执行（源自 D4 G9）
...
### ⬜ WP-124 · CTP 断线重连 SOP 实现
- **时点**：M2-M4（行情通道；Stage B 再扩展到交易通道）
- **负责**：你
- **交付**：心跳机制（10s）+ 三级响应（L1/L2/L3）+ 指数退避（2/4/8/16/32/64/128s）+ 状态指示灯
- **DoD**：模拟断网测试通过、用户感知符合设计
- **锚点**：D3 §3 CTP 断线重连 SOP
...
- **时点**：M1 起每月 standup 时
- **负责**：你 + 合伙人
- **交付**：D3 §7 11 项风险的概率/影响/触发信号月度 review 并更新（含创始人关键人风险 · D4 G10）
- **DoD**：每月 standup 纪要包含风险表 review 结果
- **锚点**：D3 §7、§8、D4 G10

### ⬜ WP-133 · 埋点 schema 与上报链路（源自 D4 G2）
- **时点**：M1 末前启用
- **负责**：你
...
| 2026-04-24 | v1.1 | 新增 6 WP（WP-63/64/84/94/95/133）· 扩展 4 WP（WP-02/05/91/130/132）· 总数 55→61 | D4 补遗 12 条 gap 落地：G1 CloudKit / G2 埋点 / G4 文华迁移 / G5 发票 / G9 pre-launch / G10 创始人风险 |
